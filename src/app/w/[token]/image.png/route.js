...
export async function GET(request, { params }) {
    ...
    const settings = {
        ...
        customBackgroundUrl: (currentUser.plan && currentUser.plan !== 'free') ? (databaseSettings.customBackgroundUrl || null) : null,
    };
    ...
    if (settings.customBackgroundUrl) {
        try {
            bgImage = await loadImage(settings.customBackgroundUrl);
        } catch (e) {
            console.warn('[route.js] Failed to load custom background image:', e.message);
        }
    }
    drawBackground(canvasContext, canvasWidth, canvasHeight, activeTheme, bgImage);
