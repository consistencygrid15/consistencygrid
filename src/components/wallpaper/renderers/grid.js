...
export function drawGrid(
    ...
    // Estimate total grid height for frosted card
    let gridHeightEst = 400;
    ...
    if (mode === "days") {
        ...
        const dayRows = Math.ceil((isLeap ? 366 : 365) / gridCols);
        gridHeightEst = dayRows * (boxSize + gap) + 30;
    } else if (mode === "weeks") {
        const columns = 13;
        const gap = 16;
        const boxSize = (contentWidth - gap * (columns - 1)) / columns;
        gridHeightEst = 4 * (boxSize + gap) + 40;
    } else if (mode === "life") {
        const gridCols = 52;
        const gap = 5;
        const rowsNeeded = Math.ceil((lifeExpectancy * 52) / gridCols);
        const availableHeight = Math.max(400, height - yCoordinate - 150);
        ...
        gridHeightEst = rowsNeeded * (boxSize + gap) + 20;
    } else if (mode === "month" || (!mode)) {
        const columns = 7;
        const gap = 20;
        const boxSize = (contentWidth - gap * (columns - 1)) / columns;
        gridHeightEst = 6 * (boxSize + gap) + 80;
        ...
        const rowHeight = 70;
        const pBoxHeight = Math.max(80, todayReminders.length * rowHeight + 20);
        const tableHeight = pBoxHeight + 40;

        const DAYS = 7;
        const GAP = 16;
        const bWidth = (contentWidth - GAP * (DAYS - 1)) / DAYS;
        const stripHeight = (bWidth * 1.2) + 24;

        gridHeightEst = stripHeight + tableHeight + 40;
    } else {
